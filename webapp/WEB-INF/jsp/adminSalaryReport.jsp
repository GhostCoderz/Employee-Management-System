...
<%@page import="com.ghostcoderz.ems.entity.Salary"%>
		...
		<table class="table table-striped table-dark table-hover text-center">
			...
			<%
					...
					for(int i=0; i<empList.size();i++)
					{
						...
						for(Salary s:empList.get(i).getSalary())
						{
			...
			%>
				<tr>
					...
					<td><%= s.getYear() %></td>
					<td><%= s.getAmount() %></td>
					<td><a href="updateSalary.jsp?id=<%= s.getSalId() %>">Edit</a></td>
					<td><a href="DeleteSalary?salId=<%= s.getSalId() %>&empId=<%=e.getId() %>">Delete</a></td>
				</tr>
			<%
						}
